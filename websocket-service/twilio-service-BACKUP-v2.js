  ...
  async makeCall({ to, prospectName, region, scenario, callType, traderProfile, leadId }) {
    try {
      ...
      const callData = {
        ...
        zohoLead: zohoLead || null, // NEW: Store Zoho lead data
        leadId: leadId || null, // NEW: Store lead ID for post-call sync
        conversationHistory: [],
  ...
  async _getClaudeResponse(callData, userSpeech) {
    ...
    try {
      ...
      const systemPrompt = callData.traderProfile
        ? this._buildDynamicPrompt(callData.prospectName, callData.scenario, callData.traderProfile, callData.zohoLead)
      ...
      if (data.content && data.content[0]) {
        ...
        if (scoreData && scoreData.score !== undefined) {
          ...
          if (callData.leadId && this.zoho.isEnabled()) {
            // Fire and forget - don't wait for Zoho update
            this.zoho.updateIntentScore(
              callData.leadId,
              callData.intentScore,
              scoreData.signal,
              scoreData.signal_type
            ).catch(err => {
              console.error('[Twilio] Zoho score update failed:', err.message);
            });
          }
        }
